...
async function loadData() {
  ...
  try {
    // 出走表（必須）と直前情報（任意）を並行取得
    const [progRes, prevRes] = await Promise.all([
      fetch(PROGRAMS_URL, { cache: "no-store" }),
      fetch(PREVIEWS_URL, { cache: "no-store" }).catch(() => null),
    ]);
...
function predict(race, preview) {
  const items = race.boats.map((b) => {
    const pv = preview && preview.boats ? preview.boats[String(b.racer_boat_number)] : null;
    return analyzeBoat(b, pv);
  });
  const n = items.length;

  // 直前：展示タイムをレース内で相対評価して加点（速いほど高評価）
  const exTimes = items.map((x) => x.pv && x.pv.racer_exhibition_time).filter((t) => t != null);
  if (exTimes.length >= 2) {
  ...
  const hasPreview = !!(preview && preview.boats);
  return {
    ranked: items, combos, hasPreview, preview,
    comment: raceComment(items, hasPreview),
  };
}
...
function raceComment(ranked, hasPreview) {
  ...
  let txt = hasPreview ? "【直前情報を反映】" : "【出走表のみ・直前情報待ち】";
...
function renderResult(race, pred) {
  const { ranked, combos, comment, hasPreview, preview } = pred;
  ...
  html += hasPreview
    ? '<span class="pre-badge on">直前情報あり</span>'
    : '<span class="pre-badge off">直前情報まだ</span>';
  html += "</div>";
...
function boatCard(x, i, maxP1) {
  ...
  if (x.pv) {
    ...
    h += '<span class="pre">展示T: <b>' + (x.pv.racer_exhibition_time != null ? x.pv.racer_exhibition_time.toFixed(2) : "-") + "</b></span>";
