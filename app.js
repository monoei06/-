...
const APP_VERSION = "2026-06-14 本命を12点フォーメーションに (v16)";
...
// 目標点数にいちばん近い3連単フォーメーションを選ぶ（着順別に候補を選択）。
function pickFormationByPoints(combos, ranked, target) {
  const idx = ranked.map((_, i) => i);
  const ord2 = [...idx].sort((a, b) => ranked[b].p2 - ranked[a].p2);
  const ord3 = [...idx].sort((a, b) => ranked[b].p3 - ranked[a].p3);
  let best = null;
  for (const n1 of [1, 2]) for (const n2 of [2, 3, 4]) for (const n3 of [3, 4, 5]) {
    ...
    if (pts > 24) continue;
    const hit = formationHitRate(combos, ranked, s1, s2, s3);
    const cand = { s1, s2, s3, pts, hit };
    if (!best) { best = cand; continue; }
    const db = Math.abs(best.pts - target), dc = Math.abs(cand.pts - target);
    if (dc < db || (dc === db && cand.hit > best.hit)) best = cand;
  }
  return best;
}

// 12点を本命に、絞り(約6点)と手広く(約18点)を添えて提案する。
function renderBets(pred) {
  const { ranked, combos } = pred;
  const specs = [
    { target: 6, label: "絞り（約6点）" },
    { target: 12, label: "本命・12点", rec: true },
    { target: 18, label: "手広く（約18点）" },
  ];
  let h = "";
  const seen = new Set();
  for (const sp of specs) {
    const t = pickFormationByPoints(combos, ranked, sp.target);
    if (!t) continue;
    const sig = t.pts + ":" + t.s1 + "|" + t.s2 + "|" + t.s3;
    if (seen.has(sig) && !sp.rec) continue; // 同じ買い目の重複は省略（本命は必ず表示）
    seen.add(sig);
    h += formationBox(sp.label, t, ranked, !!sp.rec);
  }
  return h;
}
